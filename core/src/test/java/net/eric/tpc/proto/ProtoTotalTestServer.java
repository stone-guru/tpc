...
public class ProtoTotalTestServer {
    ...
    public Service createAndStartServer(int port, String tmName) {
        List<RequestHandler> handlers = RequestHandlers.getBasicHandlers(new IntTransactionManager(tmName));
        Service service = new MinaTransService<Integer>(port, handlers, ImmutableList.of(new IntCodec()));
        service.startAsync();
